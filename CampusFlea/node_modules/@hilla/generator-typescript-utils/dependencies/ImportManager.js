...
export class NamedImportManager extends StatementRecordManager {
    ...
    *statementRecords() {
        for (const [path, specifiers] of this.#map) {
            ...
            yield [
                path,
                ts.factory.createImportDeclaration(undefined, ts.factory.createImportClause(false, undefined, ts.factory.createNamedImports(names.map((name) => {
...
export class NamespaceImportManager extends StatementRecordManager {
    ...
    *statementRecords() {
        for (const [path, id] of this.#map) {
            yield [
                path,
                ts.factory.createImportDeclaration(undefined, ts.factory.createImportClause(false, undefined, ts.factory.createNamespaceImport(id)), ts.factory.createStringLiteral(path)),
            ];
        }
    }
}
export class DefaultImportManager extends StatementRecordManager {
    ...
    *statementRecords() {
        for (const [path, { id, isType }] of this.#map) {
            yield [
                path,
                ts.factory.createImportDeclaration(undefined, ts.factory.createImportClause(isType, id, undefined), ts.factory.createStringLiteral(path)),
            ];
        }
    }
}
