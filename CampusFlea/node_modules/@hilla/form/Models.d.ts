...
export declare function getBinderNode<M extends AbstractModel<any>, T extends ModelValue<M>>(model: M): BinderNode<T, M>;
...
type ModelVariableArguments<C> = C extends new (parent: ModelParent<any>, key: keyof any, ...args: infer R extends [boolean, ...any]) => AbstractModel<any> ? R : never;
type ModelCtor = {
    new (parent: ModelParent<any>, key: keyof any, ...args: any[]): any;
    createEmptyValue(): unknown;
};
export type ModelInstance<C extends ModelCtor, MArgs extends ModelVariableArguments<C>> = C extends new (parent: ModelParent<any>, key: keyof any, ...args: MArgs) => infer M ? M : never;
...
export declare class ObjectModel<T> extends AbstractModel<T> {
    ...
    static createEmptyValue(): {
        readonly [key in never]: unknown;
    };
    private [_properties];
    protected [_getPropertyModel]<KChild extends keyof T, C extends ModelCtor, MArgs extends ModelVariableArguments<C>, M extends ModelInstance<C, MArgs>>(key: KChild, ValueModel: C, valueModelArgs: MArgs): M;
}
export declare class ArrayModel<T extends ModelValue<M>, M extends ModelInstance<C, MArgs>, C extends ModelCtor = ModelConstructor<T, M>, MArgs extends ModelVariableArguments<C> = ModelVariableArguments<C>> extends AbstractModel<readonly T[]> {
    static createEmptyValue(): [];
    private readonly [_ItemModel];
    private readonly itemModelArgs;
    private readonly itemModels;
    constructor(parent: ModelParent<readonly T[]>, key: keyof any, optional: boolean, ItemModel: C, itemModelArgs: MArgs, ...validators: ReadonlyArray<Validator<readonly T[]>>);
    /**
