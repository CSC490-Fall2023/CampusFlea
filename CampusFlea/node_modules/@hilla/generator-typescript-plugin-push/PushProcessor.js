...
export class PushProcessor {
    ...
    constructor(source, operations) {
        ...
        this.#subscriptionId = memoize(() => imports.named.add(paths.createBareModulePath('@hilla/frontend', false), 'Subscription'));
    }
    process() {
        ...
        if (this.#operations.removeInitImport) {
            const importHillaFrontend = importStatements.find((statement) => ts.isImportDeclaration(statement) &&
                statement.moduleSpecifier.text === '@hilla/frontend');
    ...
    #removeInitImport = (importStatement) => {
        ...
        if (namedImports && ts.isNamedImports(namedImports)) {
            ...
            const updatedImportClause = ts.factory.updateImportClause(importStatement.importClause, false, // FIXME: could be true, but it is false for regular endpoint calls, so sticking to that for now
            undefined, ts.factory.createNamedImports(updatedElements));
    ...
    #updateFunction(declaration) {
        ...
        return ts.factory.createFunctionDeclaration(undefined, // no async
